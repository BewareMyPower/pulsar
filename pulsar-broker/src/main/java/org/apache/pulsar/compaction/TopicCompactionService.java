...
public interface TopicCompactionService extends AutoCloseable {
    ...
    /**
     * Finds the position of the first entry in the log that has a publish time greater than or equal to the specified
     * target publish time.
     *
     * @param publishTime The target publish time in milliseconds to search for.
     * @return A future that completes with the position of the first entry whose publish time is greater than or equal
     *         to the specified `publishTime`. If no such entry exists, return the last entry.
     */
