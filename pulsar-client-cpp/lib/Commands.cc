...
SharedBuffer Commands::newSubscribe(const std::string& topic, const std::string& subscription,
                                    ...
                                    pulsar::CommandSubscribe_SubType subType, const std::string& consumerName,
                                    ...
                                    pulsar::CommandSubscribe_InitialPosition subscriptionInitialPosition,
    ...
    if (subType == pulsar::CommandSubscribe_SubType_Key_Shared) {
...
SharedBuffer Commands::newAck(uint64_t consumerId, const MessageIdData& messageId,
                              pulsar::CommandAck_AckType ackType, int validationError) {
    BaseCommand cmd;
...
std::string Commands::messageType(pulsar::BaseCommand_Type type) {
    switch (static_cast<pulsar::proto::BaseCommand_Type>(type)) {
